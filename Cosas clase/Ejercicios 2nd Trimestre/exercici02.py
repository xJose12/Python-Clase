...
def crearHTML (data, num_records):
    with open('informacion_sistema.html', 'w') as html_file:
        ...
        html_file.write('<title>Informacion del Sistema</title>\n')
...
data, num_records = sacarInformacion(ip, port, username, password)  # Aquí asignas el resultado de sacarInformacion a data y num_records

if output == 'TODO':
    crearCSV(data, num_records)
    crearHTML(data, num_records)
elif output == 'CSV':
    crearCSV(data, num_records)
elif output == 'HTML':
    crearHTML(data, num_records)
